...
class Grid extends VDOM.Component {
    ...
    onDragMeasure(e) {
        let bounds = this.clipEl.getBoundingClientRect();
        ...
        let row = Math.round((cy - bounds.top) / this.boxSize - size.height / 2);
        let col = Math.round((cx - bounds.left) / this.boxSize - size.width / 2);
        let {data} = this.props;
    ...
    componentDidUpdate() {
        ...
        this.boxSize = 25 * scale;

        this.clipEl.style.width = `${Math.round(data.columns * this.boxSize)}px`;
        this.clipEl.style.height = `${Math.round(data.rows * this.boxSize)}px`;
