...
const createOnWidgetDrop = getIndex => (e, { store }) => {
	...
	if (index == -1)
		...
		e.source.store.update("$record.widgets", items =>
			items.filter(item => item != widget)
		);
		store.update("$record.widgets", insertElement, getIndex(store), newEl);
	}
};
	...
	<cx>
		...
		<div controller={PageControlller}>
			<DropZone
				...
				inflate={300}
			/>
			<Repeater records:bind="rows" keyField="id" indexAlias="$rowIndex">
				{Row}
				<DropZone
					mod="block"
					onDropTest={e => e.source.data.type == "row"}
